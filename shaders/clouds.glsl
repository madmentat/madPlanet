...
float lowCloudSuitability(float h,float temp,float moist,float lat){
  ...
  float suit=mix(0.30+0.68*humidLand,0.98,ocean);
  suit*=1.0-0.72*desert;
  suit*=1.0-0.28*polar*(1.0-ocean*0.55);
  return clamp(suit,0.10,1.0);
}
...
/* ---------- влага вдоль наветренного следа ----------
   ...
   считаются один раз, а по следу берётся только то, что действительно
   рвётся на берегу, — макроконтур суши. Граница цикла из uDraft, поэтому
   тело отсчёта попадает в шейдер один раз, а не по копии на отсчёт. */
float lowCloudClimate(vec3 dir){
  ...
  for(int i=0;i<taps;i++){
    float fi=float(i);
    float w=exp(-fi*0.62);
    float h=continentH(normalize(dir-drift*((fi+ph)*0.050*(1.0+0.32*fi))));
    acc+=w*lowCloudSuitability(h,tempBase-max(h,0.0)*0.95,moist,lat);
    wsum+=w;
  }
...
float cumulusRegion(vec3 p,float climate){
  ...
  float threshold=mix(0.76,0.37,a);
