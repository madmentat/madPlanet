...
vec3 lightningGlow(vec3 dirW, float cloudA){
  ...
  for(int i=0;i<6;i++){
    ...
    /* Гроза идёт и днём, и разряд бьёт одинаково на обеих сторонах: раньше
       дневная сторона была запрещена наглухо, потом приглушена, а теперь
       вспышка везде одна и та же. */
    ...
    acc += vec3(0.72,0.80,1.0) * g * win * mix(2.5, 13.0, uStormGlow);
  }
  return acc*gate;
}
