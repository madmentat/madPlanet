...
vec3 shadeSurface(vec3 pos, vec3 rd, float tHit, out float dayOut){
  ...
  /* Альпийское похолодание. Общий высотный градиент нарочно пологий, иначе
     ...
     Крутизна этого градиента и есть высота снеговой линии: при uSnowAlt=0
     склон выстывает быстро и белеют уже предгорья, при 1 снег держится
     только на самых вершинах. */
  ...
  /* Углекислый газ - сырьё фотосинтеза, и при его избытке растительность
     заходит туда, где иначе была бы степь. Отсчёт от земных 0.04%: ниже
     этого уровня прибавки нет, к нескольким процентам зелень заметно
     гуще. В сухих поясах прибавка не работает - там не хватает воды,
     а не углерода. */
  ...
  if(nightF > 0.01 && uCity > 0.01 && land > 0.01){
    ...
    if(gate > 0.003){
      ...
      /* На свежих лавовых полях и вокруг действующих жерл городов не
         бывает: раньше огни спокойно загорались поверх раскалённого
         базальта. */
