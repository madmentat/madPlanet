...
# stamp is filled in here rather than duplicated in the source shell.
{
  ...
  printf '\nconst AURORA_FRAG = `%s`;\n' "$AURORA_FRAG"
  printf '\nconst SKY_FRAG = `%s`;\n' "$SKY_FRAG"
  for f in js/gl-init.js js/math.js js/hydrology.js js/state.js js/camera.js js/magnetosphere.js js/ui.js js/param-model.js js/screenshot.js js/render.js; do cat "$DIR/$f"; printf '\n'; done
  printf '</script>\n</body>\n</html>\n'
} > "$OUT"
