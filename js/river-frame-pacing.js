...
(function installRiverVisualPolish(){
  ...
  /* The only caller (riverGpuPaintVisualBranch in river-gpu.js) invokes this
     with 4 arguments and never supplies a/b node scratch vectors, so this
     patch owns and reuses its own - relying on caller-supplied a/b crashed
     weatherCoreCreate() with "Cannot set properties of undefined" as soon as
     any visual branch existed. */
  const visualEdgeA={x:0,y:0,z:0},visualEdgeB={x:0,y:0,z:0};
  riverGpuPaintVisualEdge=function(core,branch,p,tmp,a,b){
    a=a||visualEdgeA;b=b||visualEdgeB;
