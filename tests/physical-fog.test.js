...
ctx.fogStep(c,300);
assert.ok(c.fogState[0]>0&&c.fogState[0]<0.30,'saturated calm fog must form gradually, not pop in one tick');
...
assert.ok(c.fogState[0]>0.45,'persistent saturated calm conditions must build a mature fog bank');
...
ctx.fogStep(c,300);
assert.ok(c.fogState[0]<0.8&&c.fogState[0]>0.35,'dry strong wind must erode fog but never delete a bank in one fixed tick');
assert.ok(c.fogDissipationWeight[0]>0.8,'strong dry mixing must be a strong fog disperser');

/* GPU bridge: exactly two cubemaps, fixed-tick uploads only, smooth blend. */
...
gcore.fogOpticalDepth[0]=0.8;gcore.fogDepthM[0]=450;gcore.fogFormationWeight[0]=0.7;
