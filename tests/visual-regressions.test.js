...
/* 0.5.8: clouds disperse gradually over dry zones. */
...
/* Lightning is a physical strike. Cloud alpha may shape surrounding glow but
   must not attenuate the entire direct channel or make Details a lightning switch. */
assert.ok(!/if\(storm < 0\.09\) return acc;/.test(lightning), 'hard lightning cut-offs must not return');
assert.match(lightning, /float cloudLight=/, 'cloud illumination coupling missing');
assert.match(lightning, /return acc;/, 'direct lightning must not be globally multiplied by cloud alpha');
assert.ok(!/lightningGlow\([^\n]*\)\s*\*\s*\([^\n]*cl0\.a/.test(stripComments(main)), 'main must not attenuate lightning by detail-dependent cloud alpha');
