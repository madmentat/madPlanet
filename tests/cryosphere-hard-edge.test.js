...
assert.match(src,/CRYOSPHERE_EDGE_DISPLAY_MODEL=6/,'geographic (unwarped) hard-edge display model must stay active');
...
/* 0.5.108 removed the 70% circular shortcut. 0.5.109 hid the remaining circle
   behind a display-side meridional warp of the sampling direction, which
   detached ice from its own coastlines. 0.5.112 moves the geography into the
   physics (ocean-heat-transport.js); the display must sample the true
