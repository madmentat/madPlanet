...
const world={axis:[0,1,0],surfOff:0,cycA:new Float32Array(20),cycB:new Float32Array(20)};
...
const ctx={console,Math,Number,Promise,Float32Array,world,state,cam,lightningShotTrigger,
  SPIN:0,t0:1000,fullShaderDone:true,shotCaptureBusy:false,
  drawFrame(){baseFrames++;},takeShot(){shots++;return null;},
...
assert.equal(lightningShotTrigger.threshold,0.012,'trigger threshold must detect real visible physical flashes');
assert.match(triggerSrc,/0\.65\+4\.15\*stormRate\*stormRate/,'trigger must mirror renderer nonlinear flash-rate scale');
assert.match(shaderSrc,/0\.65 \+ 4\.15\*uStormRate\*uStormRate/,'renderer flash-rate scale changed without trigger sync');
assert.match(triggerSrc,/0\.85\+1\.45\*storm\*storm/,'trigger must mirror renderer activity scale');
assert.match(shaderSrc,/0\.85 \+ 1\.45\*uStorm\*uStorm/,'renderer activity scale changed without trigger sync');
assert.match(triggerSrc,/24\.0-2\.8/,'trigger glow range must track renderer maximum');
assert.match(shaderSrc,/mix\(2\.8,24\.0,uStormGlow\*uStormGlow\)/,'renderer glow range changed without trigger sync');
...
cam.dist=3;setStormDirection(0,0,1);
