...
/* 0.5.85 regression: comments may describe the retired value, but executable
   code must not create a hard |belt.x| activation contour around tectonics. */
assert.doesNotMatch(terrain,/if\s*\([^\n)]*abs\s*\(\s*belt\.x\s*\)[^\n)]*>\s*0\.004/,
